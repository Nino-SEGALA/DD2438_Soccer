...
public class DroneAIBTSoccer_blue : MonoBehaviour
{
    ...
    /*private bool ClearancePossible()
    {
        if (CanKick())  // close enough to ball
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                Vector3 directionForShooting = 0.5f * (enemy1-ballPos) + 0.5f * (enemy2-ballPos);  // bisector
                ...
                //Debug.Log("ctbfas : directionForShooting = " + directionForShooting);
                if (areBlue)
                {
                    if (directionForShooting.x > bestDirectionForShooting.x)  // shoot to the right
                    {
                ...
                {
                    if (directionForShooting.x < bestDirectionForShooting.x)  // shoot to the left
                    {
                        bestDirectionForShooting = directionForShooting;
                        res = true;
                    }
        ...
        if (CanKick())  // close enough to ball
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                ...
                if ((areBlue && directionForShooting.x > 0) || (!areBlue && directionForShooting.x < 0))  // shoot in good direction
                {
                    ...
                    /*if (minDirShootAngleBetween > maxAngleBetween)
                    {
                        maxAngleBetween = minDirShootAngleBetween;
                        bestDirectionForShooting = directionForShooting;
                        res = true;
                    }*/
    ...
    void Challenge()
    {
        ...
        if (distance(transform.position, objective) > distanceChallenge)  // far away from ball, take into account velocity
        {
            objective = objective + ballSpeed;
        }

        objective.x = objective.x + ballDiameter / 2 * axis.x;  // TODO: velocity -> goal: go_side
    ...
    bool FrontPost()  // Return true if current agent should be at fronpost
    {
        ...
        GameObject other = null;  // GameObject other = transform.gameObject;  //  transform.position;
        foreach (GameObject friend in friends)
        {
            ...
            else if (Math.Abs((friend.transform.position - ball.transform.position).magnitude - minDist) == 0)  // < 0.1)
            {
                continue;
            }
            other = friend;
